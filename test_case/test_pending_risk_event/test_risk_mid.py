    ...
    def setup_class(cls):
        ...
        try:
            with allure.step('【风险】-【风险事件列表】接口'):
                ...
                cls.base_url = risk_list_all_pending(sc_url=sc_ip, headers=cls.headers, risk_level='3')
                logger.info(
                    f'获取到风险列表中危待处置数量：{cls.base_url["pending_total_count"]}，已处置数量：{cls.base_url["total_count"]}')
