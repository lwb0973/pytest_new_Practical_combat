...
class TestCase:
    ...
    def test_vul_high_pending(self, sc_login):
        """待处置/已处置高危弱点事件饼图总数"""
        try:
            ...
            with allure.step('【弱点】-【弱点事件列表】接口'):
                ...
                logger.info(f'获取弱点列表高危待处置数量：{pending_total}，已处置数量：{total_count}')

                assert pending_total is not None, "弱点列表待处置高危弱点事件总数未获取"
                assert total_count is not None, "弱点列表高危弱点事件总数未获取"
            ...
            with allure.step('校验待处置数一致'):
                assert home_data.get("pending_count") == pending_total, (
                    f'待处置数不一致：首页待处置高危弱点为 {home_data.get("pending_count")}，'
                    f'弱点列表待处置高危弱点为 {pending_total}'
                )

            # 校验已处置数量
            with allure.step('校验已处置数一致'):
                assert home_data.get("done_count") == total_count, (
                    f'已处置数不一致：首页已处置高危弱点为 {home_data.get("done_count")}，'
                    f'弱点列表已处置高危弱点为 {total_count}'
                )

        except Exception as e:
            logger.error(f'test_vul_high_pending 异常: {e}', exc_info=True)
