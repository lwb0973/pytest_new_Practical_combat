...
class TestCase:
    ...
    @allure.story('【风险账号总数】')
    def test_risk_user(self, sc_login):
        """首页风险概览风险账号总数"""
        try:
            ...
            with allure.step('首页风险账号总数与用户账号列表总数对比一致'):
                assert user_risk_total == user_row_count, (
                    f'首页风险账号总数({user_risk_total})与用户账号列表总数({user_row_count})不一致'
                )
