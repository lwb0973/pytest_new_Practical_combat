...
@allure.feature('【首页】-【风险概览】-【风险账号总数】')
class TestCase:
    @classmethod
    def setup_class(cls):
        logger.info('========获取首页风险概览风险账号总数数据，测试开始==========')
        try:
            with allure.step('【用户】-【账号列表】接口'):
                url = f'https://{sc_ip}/apione/v2/user/accounts/list?page_num=1&page_size=10&risk_levels=2%2C3%2C4'
    ...
    def teardown_class(cls):
        logger.info('========获取首页风险概览风险账号总数数据，测试结束==========')

    @allure.severity('normal')
    @allure.story('【风险账号总数】')
    def test_risk_user(self):
        try:
            with allure.step('【风险账号总数】接口'):
                url = f'https://{sc_ip}/apione/v2/user/accounts/metrics'
                response = requests.request(method='GET', url=url, headers=self.headers, verify=False)
                assert response.status_code == 200, '首页风险账号总数请求响应失败'
                ...
                assert user_risk_total is not False, '未获取风险账号总数字段数据'
                assert self.user_row_count is not False, '未获取账号列表总数字段数据'
            with allure.step(f'首页风险账号总数和用户-账号列表总数对比一致,首页总数:{user_risk_total},账号列表总数:{self.user_row_count}'):
                assert user_risk_total == self.user_row_count, '首页风险账号总数与用户列表的风险账号总数不一致'

        except JSONDecodeError as e:
            logger.error(f'提取JSON解析风险账号总数错误{e}')
            pytest.fail(f'提取JSON解析风险账号总数解析失败: {e}')
        except Exception as e:
