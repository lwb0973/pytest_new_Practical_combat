...
@allure.feature('【首页】-【风险概览】-【弱点api总数】')
class TestCase:
    @classmethod
    def setup_class(cls):
        logger.info('========获取首页风险概览弱点api总数数据，测试开始==========')
        try:
            with allure.step('【资产】-【api列表】接口'):
                url = f'https://{sc_ip}/apione/v2/assets/list'
                payload = {"page_num": 1, "page_size": 10, "sort": -1, "time_layout": "2006-01-02 15:04:05",
                           "vul_level_ids": [2, 3, 4]}
    ...
    def teardown_class(cls):
        logger.info('========获取首页风险概览弱点api总数数据，测试结束==========')

    @allure.tag('smoke','test_vul_total')
    @allure.severity('normal')
    @allure.story('【弱点api总数】')
    def test_vul_total(self):
        try:
            with allure.step('【弱点api总数】接口'):
                ...
                assert response.status_code == 200, '首页弱点api总数请求响应失败'
                metric = jsonpath(response.json(), '$.data.metric')[0]
                risk_high = metric.get('vul_high_count', 0)
                risk_mid = metric.get('vul_mid_count', 0)
                risk_low = metric.get('vul_low_count', 0)
            ...
            logger.error(f'提取JSON解析弱点api总数错误{e}')
            pytest.fail(f'提取JSON解析弱点api总数解析失败: {e}')
        except Exception as e:
