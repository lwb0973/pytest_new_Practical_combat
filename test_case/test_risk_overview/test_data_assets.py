...
@allure.feature('【首页】-【风险概览】-【涉敏流动数据量】')
class TestCase:
    @classmethod
    def setup_class(cls):
        logger.info('========获取首页风险概览涉敏流动数据量，测试开始==========')

    @classmethod
    def teardown_class(cls):
        logger.info('========获取首页风险概览涉敏流动数据量，测试结束==========')

    @allure.severity('normal')
    @allure.story('【涉敏流动数据量】')
    def test_data_assets(self, sc_login):
        ...
        try:
            ...
            with allure.step('【涉敏流动数据量】接口'):
                url = f'https://{sc_ip}/apione/v2/data-assets/data-count'
                response = requests.get(url, headers=headers, verify=False)
                assert response.status_code == 200, '首页涉敏流动数据量请求响应失败'
                sens_count = jsonpath(response.json(), '$.data.sens_data_count')
                assert sens_count, '未获取涉敏流动数据量字段数据'
                sens_count = sens_count[0]

            with allure.step('首页涉敏流动数据量和资产-流动数据列表涉敏流动数据量对比一致'):
                assert sens_count == data_count, (
                    f'首页涉敏数据量({sens_count})与数据列表涉敏流动数据量({data_count})不一致'
                )

        except JSONDecodeError as e:
            logger.error(f'提取JSON解析涉敏流动数据量错误: {e}')
            pytest.fail(f'提取JSON解析涉敏流动数据量解析失败: {e}')
